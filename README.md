...
AI 翻译流程：发现可见内容 → 查段落缓存 → 文本指纹与请求复用 → 25ms 内合批 → 按前台优先级、并发和额度派发 → 读取 Responses 流 → 首个有效译文立即显示，后续更新最多每 80ms 合并绘制。已收到的流式译文会立即回放给重新出现的组件；词汇生成不阻塞已收到的正文显示。页面保留 6 个请求并发，预加载最多占 2 个。
