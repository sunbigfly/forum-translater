<div align="center">
  ...
  <img alt="版本" src="https://img.shields.io/badge/version-0.2.11-536af5">
...
字体设置：在翻译设置的「字体」页分别选择标题、正文字体和字号，支持本机字体搜索、中文名称、预览、手动填写与恢复默认。切到「字体」页时自动读取本机字体，并按需触发浏览器授权；没有授权也可使用预设或填写名称。保存后同步应用于原文和译文，不重启翻译、不清除缓存；字号留空时跟随网站，代码保留等宽字体。
...
- X 长帖默认只翻译预览内容，并补全预览末尾被截断的句子；其余内容点击 Show more 后才翻译，展开与收起保留已有译文。
...
- 移动端使用可用正文宽度，互动栏保持单行；头像与占位列同步缩小，避免与用户名、正文重叠。
...
- 桌面端与移动端共用返回优化：关闭时保留已有译文，后台页面就绪后立即恢复阅读位置；主动滚动时取消校正。
...
X 动态内容引发的帖子、媒体、导航和广告扫描也在滚动停止后分批执行，复用已识别的媒体容器，减少重复扫描。

词汇高亮更新避免触发页面重扫，例句更新复用已有标记；移动端和触屏保留高亮但不弹出悬停词卡。译文过滤媒体留下的空白容器，并清理完整译文的首尾空白。
...
AI 翻译流程：发现可见内容 → 查段落缓存 → 文本指纹与请求复用 → 25ms 内合批 → 按前台优先级、并发和额度派发 → 读取 Responses 流 → 按内容合并更新并分批绘制变化的段落。缓存译文及时显示；流式刷新以 80ms 窗口合并，不同内容之间让出主线程。已完成段落和未变化的静态占位保留原节点，流式片段不反复重做词汇高亮；新增无关帖子不会重新扫描正在翻译的内容。已收到的流式译文会回放给重新出现的组件，词汇生成不阻塞正文显示。页面保留 6 个请求并发，预加载最多占 2 个。

点击重试只补失败段落，连续点击复用进行中的任务。批次中已完整返回并通过校验的正文立即缓存，即使后续段落或词汇生成失败，也不重新翻译这部分正文；词汇缓存缺失时只补词汇，并复用正在进行的词汇请求。自动重试退避结束后再次检查缓存，避免其他请求已经成功却又重发。

「翻译性能统计」中的 `section.firstContentMs` 包含客户端准备、排队及接口等待；`translation.firstContentMs` 从接口请求开始计时，JSON 格式前缀不算首字。汇总的 `firstContentP50Ms` / `firstContentP95Ms` 与总完成耗时分开统计；没有首字的失败请求不会被计作 0ms。
